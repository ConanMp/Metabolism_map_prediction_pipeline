...
def is_node_linked_to_root(node, nodes_depencies_dic):
    node_dependencies=nodes_depencies_dic[node]
...
def new_sort_case(node_parents_cases, final_node, is_linked_to_root):
    new_cases=[]
    if node_parents_cases==[]:
        new_cases=[final_node]
        return new_cases
    if not is_linked_to_root:
...
def calculate_node_probability(node, node_dependencies, enzyme_cases, is_linked_to_root):
    print("===========================================")
    print("\t\t\t"+str(node))
    print("===========================================")
    ...
    sorted_parents_cases=new_sort_case(all_parents_cases, node, is_linked_to_root)
    ...
    final_combiation_nodes_parents={}
    for an_enzyme_case2 in enzyme_sorted_cases.keys():
            final_combiation_nodes_parents[an_enzyme_case2]={}
            ...
            for a_case in node_cases_list:
                ...
                for letter in a_splitted_case:
                    ...
                    letter_proba, final_parents_node_combinaison = calculate_node_case_probability(present_parent, prensent_enzyme, node_dependencies[letter.replace("~","")], letter)
                    if "~" not in a_case:
                        final_combiation_nodes_parents[an_enzyme_case2][letter]=final_parents_node_combinaison
    ...
    return(node_probability_dictionnary, final_combiation_nodes_parents)
...
def read_combinations(final_combination_dic):
    readed_combination_dic={}
    for node in final_combination_dic.keys():
        node_final_combination_dic=final_combination_dic[node]
        for enzyme_case in node_final_combination_dic.keys():
            if enzyme_case not in readed_combination_dic.keys():
                readed_combination_dic[enzyme_case]={}
            combination_for_enzyme_case=node_final_combination_dic[enzyme_case]
            childs_nodes=list(combination_for_enzyme_case.keys())
            for child in childs_nodes:
                parents_list_combination=combination_for_enzyme_case[child]
                if parents_list_combination == None:
                    continue
                for parent_tuple_string in parents_list_combination:
                    parent_tuple=literal_eval(parent_tuple_string)
                    parent=parent_tuple[0]
                    enzyme=parent_tuple[1]
                    reaction=str(parent)+' - '+str(child)
                    if reaction in readed_combination_dic[enzyme_case].keys():
                        readed_combination_dic[enzyme_case][reaction].append(enzyme)
                    else:
                        readed_combination_dic[enzyme_case][reaction]=[enzyme]
    return(readed_combination_dic)


def treat_readed_final_combination(json_map_dic, readed_final_combination):
    treated_dic={}
    reaction_equivalent_dic={}
    for enzyme_case in readed_final_combination.keys():
        treated_dic[enzyme_case]={}
        reactions_combinatin_dic=readed_final_combination[enzyme_case]
        for reaction_key in reactions_combinatin_dic.keys():
            reaction_enzymes_list=list(set(reactions_combinatin_dic[reaction_key]))
            enzyme=reaction_enzymes_list[0]
            if reaction_key in reaction_equivalent_dic:
                reaction_id=reaction_equivalent_dic[reaction_key]
                treated_dic[enzyme_case][reaction_id]=enzyme
            else:
                splitted_reaction_key=reaction_key.split(" - ")
                reaction_source=int(splitted_reaction_key[0].replace('~',''))
                reaction_target=int(splitted_reaction_key[1].replace('~',''))
                reactions=json_map_dic["links"]
                for a_reaction in reactions:
                    a_reaction_source=int(a_reaction["source"])
                    a_reaction_target=int(a_reaction["target"])
                    a_reaction_id=a_reaction["id"]
                    if reaction_source == a_reaction_source and reaction_target == a_reaction_target:
                        reaction_equivalent_dic[reaction_key]=a_reaction_id
                        treated_dic[enzyme_case][a_reaction_id]=enzyme
                        break
    return(treated_dic)        
...
def write_treated_final_combination(treated_final_combination, output_directory, rank1_tool_name, rank2_tool_name):
    if output_directory[len(output_directory)-1]!="/":
        output_directory=output_directory+"/"
    output_directory=output_directory+"Probabilities_results/"+str(rank1_tool_name)+"_as_rank1_and_"+rank2_tool_name+"_as_rank2/Reactions_selected_enzymes/"
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
    for enzyme_case in treated_final_combination.keys():
        enzyme_case_final_combination=treated_final_combination[enzyme_case]
        splitted_enzyme_case=enzyme_case.split()
        enzyme_list=[]
        enzyme_to_replace=[]
        for enzyme in splitted_enzyme_case:
            if '~' not in enzyme:
                enzyme_list.append(enzyme)
            else:
                replaced_enzyme=enzyme.replace('~','')
                enzyme_to_replace.append(replaced_enzyme)
                enzyme_list.append(replaced_enzyme)
        enzyme_list=sorted(enzyme_list)
        for enzyme_index in range(0,len(enzyme_list)):
            enzyme=enzyme_list[enzyme_index]
            if enzyme in enzyme_to_replace:
                replaced_enzyme="~"+enzyme
                enzyme_list[enzyme_index]=replaced_enzyme
        enzyme_case_output=output_directory+"_".join(enzyme_list)+".tsv"
        first_line="Reaction_id\tEnzyme_Selected"
        lines=[first_line]
        for reaction_id in enzyme_case_final_combination.keys():
            enzyme=enzyme_case_final_combination[reaction_id]
            a_line=str(reaction_id)+"\t"+enzyme
            lines.append(a_line)
        output_file=open(enzyme_case_output, 'w')
        output_file.write("\n".join(lines))
...
def main():
    ...
    if is_metabolism_map_empty:
        ...
        all_combination_final={}
        for node in all_node_dependencies.keys():
            is_linked_to_root=is_node_linked_to_root(node,all_node_dependencies)
            if node=="0":
                ...
                node_probabilities, final_combinaison_dic=calculate_node_probability(node, all_node_dependencies, all_enzyme_cases, is_linked_to_root)
                general_probability_dictionnary[node]=node_probabilities
                all_combination_final[node]=final_combinaison_dic
        readed_combinations=read_combinations(all_combination_final)
        treated_final_combination=treat_readed_final_combination(metabolism_map, readed_combinations)
        write_general_probability(output_directory, general_probability_dictionnary, rank1_tool_name, rank2_tool_name)
        write_treated_final_combination(treated_final_combination, output_directory, rank1_tool_name, rank2_tool_name)
